...
class SimulationPreparator:
    ...
    def connect_redis(self):
        """Connect to Redis with password handling for remote/localhost"""
        try:
            # Check if connecting to localhost
            if self.redis_host in ['localhost', '127.0.0.1']:
                print(f"🔗 Connecting to Redis at {self.redis_host}:{self.redis_port} (no password)")
                ...
                print(f"🔗 Connecting to Redis at {self.redis_host}:{self.redis_port} (with password)")
            ...
            if self.redis_host not in ['localhost', '127.0.0.1']:
                print(f"❌ Password file '.redis_passwd' not found for remote connection")
                return False
            else:
                print(f"❌ Unexpected error: password file needed for localhost")
                return False
        ...
        try:
            ...
            for month_offset in range(months_to_fetch):
                ...
                try:
                    ...
                    if 'Note' in data:
                        print(f"   ⚠️ Rate limit hit at month {month_offset + 1}")
                        break
            ...
            if not all_data:
                print("   ⚠️ Premium monthly fetch failed, trying alternative premium endpoint...")
    ...
    def download_yahoo_data(self, symbol, days=60):
        ...
        try:
            ...
            if validation == "daily_only":
                print(f"   ⚠️ Using daily data (5-minute not available)")
    ...
    def convert_to_timesale_format(self, df, symbol, source='yahoo'):
        ...
        print(f"   🏷️ Source tagged as: {source.upper()}")
    ...
    def upload_to_redis(self, timesale_data, symbol, source='yahoo'):
        ...
        if uploaded_days < len(daily_data):
            print(f"   ⚠️ WARNING: Only {uploaded_days} of {len(daily_data)} days uploaded successfully")
