...
class ExitTimingModel(nn.Module):
    def __init__(self, input_features=8, sequence_length=15, position_features=5, cnn_filters=32, lstm_hidden=64, dropout=0.2):
        ...
        self.market_lstm = nn.LSTM(input_size=cnn_filters, hidden_size=lstm_hidden, num_layers=1, batch_first=True, bidirectional=True)
...
class TradingSignalRecorder:
    ...
    def record_entry_signal(self, timestamp, signal_type, confidence, price, market_state):
        signal = {'timestamp': timestamp, 'type': signal_type, 'confidence': confidence, 'entry_price': price, 'market_state': market_state, 'status': 'open'}
                ...
                try:
                    ...
                    for time_offset in range(5, max_hold_time, 2):
                        ...
                        if current_idx + self.sequence_length < len(features):
                            ...
                            current_price = data['close_raw'].iloc[current_idx]
                            unrealized_pnl = self._calculate_unrealized_pnl(signal['entry_price'], current_price, signal['type'])
                            volatility = data['close_raw'].iloc[max(0, current_idx-10):current_idx].std()
                            pos_features = np.array([unrealized_pnl, time_offset / 60.0, signal['confidence'], volatility / current_price if current_price > 0 else 0.01, 1.0 if signal['type'] == 'LONG' else -1.0])
...
class TwoStageTrader:
    ...
    def save_model_checkpoint(self, model, model_name, epoch=None, loss=None):
        """Save model checkpoint with metadata"""
        try:
            timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
            if epoch is not None:
                filename = f"{model_name}_{self.symbol}_epoch_{epoch}_{timestamp}.pt"
            else:
                filename = f"{model_name}_{self.symbol}_{timestamp}.pt"
            ...
            # Also save the simple version for backward compatibility
            simple_filename = f"{model_name}_{self.symbol}.pt"
            simple_filepath = os.path.join(self.checkpoint_dir, simple_filename)
            torch.save(model.state_dict(), simple_filepath)
            print(f"💾 Model state dict saved: {simple_filepath}")
            
            # Verify the file was actually saved
            if os.path.exists(filepath) and os.path.getsize(filepath) > 0:
                print(f"✅ Checkpoint verified: {os.path.getsize(filepath)} bytes")
                return filepath
            else:
                print(f"❌ Checkpoint save failed or file is empty")
                return None
    ...
    def train_entry_model(self, data, epochs=30):
        ...
        self.entry_model = EnhancedEntryModel(input_features=sequences.shape[2], sequence_length=sequences.shape[1]).to(self.device)
        ...
        for epoch in range(epochs):
            ...
            if (epoch + 1) % 10 == 0:
                print(f"Entry model epoch {epoch+1}/{epochs}: Loss: {avg_loss:.4f}")
                # Save checkpoint every 10 epochs
                self.save_model_checkpoint(self.entry_model, "entry_model", epoch+1, avg_loss)
        
        # Save final model
        final_checkpoint = self.save_model_checkpoint(self.entry_model, "entry_model", epochs, avg_loss)
    ...
    def generate_entry_signals(self, data):
        ...
        for i, (idx, row) in enumerate(signal_data.iterrows()):
            ...
            if confidence > confidence_threshold:
                if prediction == 1:
                    signal_id = self.signal_recorder.record_entry_signal(timestamp=idx, signal_type='LONG', confidence=confidence, price=row['close_raw'], market_state={'rsi': row.get('rsi_14', 50), 'volume_ratio': 1.0, 'volatility': row['close_raw'] * 0.01})
                    signal_count += 1
                elif prediction == 2:
                    signal_id = self.signal_recorder.record_entry_signal(timestamp=idx, signal_type='SHORT', confidence=confidence, price=row['close_raw'], market_state={'rsi': row.get('rsi_14', 50), 'volume_ratio': 1.0, 'volatility': row['close_raw'] * 0.01})
                    signal_count += 1
    ...
    def simulate_exits_for_training(self, data):
        ...
        for signal_id, signal in enumerate(self.signal_recorder.signals):
            if signal['status'] == 'open':
                ...
                self.signal_recorder.record_exit_signal(signal_id, best_exit_price, 'simulated')
                exit_count += 1
        print(f"✅ Simulated exits for {exit_count} signals")
    
    def train_exit_model(self, data, epochs=30):
        print("🎯 Training exit timing model...")
        exit_sequences, exit_labels, position_features = self.preprocessor.create_exit_training_data(data, self.signal_recorder.signals)
        if len(exit_sequences) == 0:
        ...
        self.exit_model = ExitTimingModel(input_features=exit_sequences.shape[2], sequence_length=exit_sequences.shape[1], position_features=position_features.shape[1]).to(self.device)
        ...
        for epoch in range(epochs):
            ...
            avg_loss = train_loss/len(train_loader)
            if (epoch + 1) % 10 == 0:
                print(f"Exit model epoch {epoch+1}/{epochs}: Loss: {avg_loss:.4f}")
                # Save checkpoint every 10 epochs
                self.save_model_checkpoint(self.exit_model, "exit_model", epoch+1, avg_loss)
        
        # Save final model
        final_checkpoint = self.save_model_checkpoint(self.exit_model, "exit_model", epochs, avg_loss)
    ...
    def integrated_backtest(self, data, initial_cash=25000):
            ...
            if confidence > 0.5:
                if prediction == 1 and 'LONG' not in positions:
                    ...
                    if shares > 0 and shares * raw_price <= cash:
                        ...
                        positions['LONG'] = {'shares': shares, 'entry_price': raw_price, 'entry_time': i, 'confidence': confidence}
                        trades += 1
                elif prediction == 2 and 'SHORT' not in positions:
                    max_shares = min(int(cash // raw_price), int(MAX_POSITION_VALUE // raw_price))
                    shares = int(max_shares * confidence * 0.4)
                    if shares > 0:
                        positions['SHORT'] = {'shares': shares, 'entry_price': raw_price, 'entry_time': i, 'confidence': confidence}
                        trades += 1
            if len(positions) > 0:
        ...
        results = {'final_return': final_return, 'total_trades': trades, 'portfolio_values': portfolio_values, 'final_positions': len(positions)}
...
def main():
    ...
    parser.add_argument('--mode', type=str, default='full', choices=['entry', 'exit', 'full'], help='Training mode')
    ...
    try:
        ...
        if args.mode == 'full':
            ...
            results_data = {'symbol': args.symbol, 'strategy_type': 'Two_Stage_Entry_Exit', 'final_return': float(results['final_return']), 'total_trades': int(results['total_trades']), 'final_positions': int(results['final_positions']), 'timestamp': datetime.now().isoformat()}
