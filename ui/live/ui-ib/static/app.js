document.addEventListener('DOMContentLoaded', () => {
    ...
    // Signal markers array - THIS IS THE KEY FIX
    let allSignalMarkers = [];

    addDebugLog('Chart initialized with candlestick series');

    // Load live data from IB Gateway
    ...
    // FIXED: Add signal markers and keep ALL of them persistent
    function addSignalMarker(action, confidence, price, candleTime) {
        ...
        // Add to the persistent array - NO LIMIT, KEEP ALL SIGNALS
        allSignalMarkers.push(marker);
        
        // Apply ALL markers to the chart
        candleSeries.setMarkers(allSignalMarkers);
        addDebugLog(`Added ${action} signal. Total signals: ${allSignalMarkers.length}`);
    }
