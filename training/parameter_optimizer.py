#!/usr/bin/env python3
"""
Enhanced Parameter Optimizer for Trading System - COMPLETE FIXED VERSION
Fixes all errors and adds proper parameter capture and reporting
"""
...
class OptimConf:
    ...
    min_trades: int = 5  # Reduced to capture more results
    max_dd: float = 0.20  # More lenient
    max_iter: int = 200
    n_jobs: int = 1

# Enhanced search space - more aggressive
...
WINDOW = [10, 15, 20]  # Keep as integers
...
class ResultsTracker:
    ...
    def add_result(self, params, bt_results, score):
        ...
        # Track highest return regardless of score
...
def enhanced_score(bt: Dict, conf: OptimConf) -> float:
    ...
    # Calculate annualized projection (assuming ~17 day test period)
    test_days = 17
    daily_return = total_return / max(test_days, 1)
    annual_return = (1 + daily_return) ** 252 - 1
    
    # Primary score based on annual return
    ...
    def _maker(df: pd.DataFrame):
        try:
            ...
            if signals:
                ...
                # Aggressive voting - any signal triggers action
...
class EnhancedGAOptimizer:
    ...
    def eval(self, params: Dict) -> float:
        ...
        try:
            # Ensure window_minutes is integer
            ...
            try:
                ...
                # Add test period info if missing
    ...
    def run(self):
        ...
        toolbox.register("win", lambda: random.choice(WINDOW))  # Ensure integer

        toolbox.register("individual", tools.initCycle, creator.Individual,
                         (toolbox.up, toolbox.dn, toolbox.conf,
                          toolbox.ps, toolbox.pt, toolbox.sl,
                          toolbox.win), 1)
    ...
    def print_enhanced_summary(self):
        """Print comprehensive results with highest return focus"""
        print(f"\n{'='*80}")
        ...
        # Get best by return (most important for your use case)
        ...
        if best_return_result['params']:
            ...
            print(")")
            print("")
            print("# Custom label thresholds:")
            print(f"up_threshold = {params['up']:.4f}")
            print(f"down_threshold = {params['dn']:.4f}")
        ...
        # Show top performers by score as well
        print(f"\n{'='*60}")
