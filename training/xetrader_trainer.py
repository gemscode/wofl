...
"""
Enhanced Advanced Hedge Fund Style Trading System with Interactive Brokers Support
Automatically loads optimized configurations for any symbol and supports IB data
"""
...
class TradingConfig:
    ...
    data_source: str = 'alpha'  # 'alpha' or 'ib'
    ib_bar_size: str = '1 min'  # IB bar size
...
def load_optimized_config_for_symbol(symbol: str) -> Optional[TradingConfig]:
    ...
    try:
        ...
        print(f"Error loading optimized config for {symbol}: {e}")
        return None

class EnhancedDataManager:
    """Enhanced data manager with IB support and proper train/test splitting"""
    
    def __init__(self):
        self.data_manager = DataManager()
    
    async def get_training_data_with_split(
        self, symbol: str, total_days: int, exclude_recent_days: int = 30,
        data_source: str = 'alpha', **kwargs
    ) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """
        Get training and testing data with proper temporal split using specified data source
        ...
        Args:
            ...
            **kwargs: Additional arguments (ib_bar_size, etc.)
            
        Returns:
            Tuple of (training_data, testing_data)
        """
        print(f"Fetching {total_days} days of {data_source.upper()} data for proper train/test split...")
        
        # Fetch all data using the new async method
        full_data = await self.data_manager.get_training_data(
            symbol, 
            total_days + exclude_recent_days, 
            data_source, 
            **kwargs
        )
        
        if full_data.empty:
            raise ValueError(f"No {data_source.upper()} data available for {symbol}")
        
        # Calculate split points based on time, not just bar count
        # This is more accurate for IB data which may have variable bar counts per day
        total_bars = len(full_data)
        
        if data_source == 'ib':
            # For IB data, use time-based splitting
            end_time = full_data.index[-1]
            train_end_time = end_time - timedelta(days=exclude_recent_days)
            
            train_data = full_data[full_data.index <= train_end_time].copy()
            test_data = full_data[full_data.index > train_end_time].copy()
        else:
            # For alpha data, use the original bar-based method
        ...
        if len(train_data) < 1000:
            raise ValueError(f"Insufficient training data. Need at least 1000 bars, got {len(train_data)}")
        
        if len(test_data) < 100:
            raise ValueError(f"Insufficient test data. Need at least 100 bars, got {len(test_data)}")
        ...
        time_gap = (test_data.index[0] - train_data.index[-1]).total_seconds() / 60
        print(f"Gap between train and test: {time_gap:.0f} minutes")
        
        return train_data, test_data
    
    async def get_backtest_data(self, symbol: str, days: int, start_date: str = None, 
                              end_date: str = None, data_source: str = 'alpha', **kwargs) -> pd.DataFrame:
        """Get data for backtesting with optional date filtering"""
        data = await self.data_manager.get_training_data(symbol, days, data_source, **kwargs)
...
class EnhancedEnsembleSignalGenerator:
    ...
    def _create_enhanced_labels(self, df: pd.DataFrame) -> pd.Series:
        ...
        """
        # Use the optimized thresholds from config
        ...
        # Define horizons with scaled thresholds
        ...
        vol_multiplier = vol_multiplier.fillna(1.0).clip(0.5, 2.0)
...
class EnhancedAdvancedTradingSystem:
    """Enhanced trading system with IB support and improved performance"""
    ...
    def __init__(self, symbol: str, config: TradingConfig = None):
        ...
        print(f"Data source: {self.config.data_source.upper()}")
        if self.config.data_source == 'ib':
            print(f"IB bar size: {self.config.ib_bar_size}")
            print(f"IB Gateway: {self.config.ib_host}:{self.config.ib_port}")
    ...
    def prepare_comprehensive_dataset(self, raw_data: pd.DataFrame) -> pd.DataFrame:
        """Enhanced dataset preparation with IB data support"""
        print(f"Preparing enhanced dataset from {len(raw_data)} raw bars...")
        
        # For IB data, we might already have the desired timeframe
        if self.config.data_source == 'ib':
            # IB data might already be in the correct timeframe, but we can still resample if needed
            if self.config.window_minutes > 1:
                # Parse IB bar size to minutes
                ib_minutes = self._parse_ib_bar_size_to_minutes(self.config.ib_bar_size)
                if ib_minutes < self.config.window_minutes:
                    rule = f"{self.config.window_minutes}T"
                    agg = {'open': 'first', 'high': 'max', 'low': 'min', 'close': 'last', 'volume': 'sum'}
                    data = raw_data.resample(rule).agg(agg).dropna()
                    print(f"Resampled IB data from {len(raw_data)} to {len(data)} {self.config.window_minutes}-minute bars")
                else:
                    data = raw_data.copy()
            else:
                data = raw_data.copy()
        else:
            # Original Alpha Vantage logic
            if self.config.window_minutes > 1:
                rule = f"{self.config.window_minutes}T"
                agg = {'open': 'first', 'high': 'max', 'low': 'min', 'close': 'last', 'volume': 'sum'}
                data = raw_data.resample(rule).agg(agg).dropna()
    ...
    def _parse_ib_bar_size_to_minutes(self, bar_size: str) -> float:
        """Parse IB bar size to minutes for comparison"""
        if 'secs' in bar_size or 'sec' in bar_size:
            seconds = int(bar_size.split()[0])
            return seconds / 60.0
        elif 'min' in bar_size:
            return int(bar_size.split()[0])
        elif 'hour' in bar_size:
            return int(bar_size.split()[0]) * 60
        else:
            return 1.0  # Default to 1 minute
    
    async def train_system(self, total_days: int = 120) -> Dict:
        """Enhanced training with IB data support"""
        print(f"\n{'='*70}")
        print(f"Training Enhanced Advanced Trading System for {self.symbol}")
        print(f"Data Source: {self.config.data_source.upper()}")
        print(f"{'='*70}")
        
        # Prepare kwargs for IB data
        data_kwargs = {}
        if self.config.data_source == 'ib':
            data_kwargs.update({
                'bar_size': self.config.ib_bar_size,
                'ib_host': self.config.ib_host,
                'ib_port': self.config.ib_port,
                'ib_client_id': self.config.ib_client_id
            })
        
        train_data_raw, test_data_raw = await self.data_manager.get_training_data_with_split(
            self.symbol, 
            total_days, 
            self.config.data_split_exclude_days,
        ...
        # Save model with IB-specific metadata
        ...
        with open(model_path, 'wb') as f:
            pickle.dump({
                ...
                'training_stats': {
                    ...
                    'ib_bar_size': self.config.ib_bar_size if self.config.data_source == 'ib' else None
                }
            }, f)
        ...
        if backtest_results['total_return'] > 0.05:
            ...
            print(f"NEEDS IMPROVEMENT: {backtest_results['total_return']:.1%} return")
...
async def main():
    parser = argparse.ArgumentParser(description='Enhanced Advanced Trading System with IB Support')
    ...
    # IB-specific arguments
    parser.add_argument('--ib', action='store_true', help='Use Interactive Brokers data')
    parser.add_argument('--bar-size', default='1 min', help='IB bar size (e.g., "1 min", "30 secs", "5 mins")')
    ...
    parser.add_argument('--ib-client-id', type=int, default=3, help='IB client ID')
    ...
    # Try to load optimized config for ANY symbol
    ...
    if config is None:
        ...
        config.ib_bar_size = args.bar_size
        ...
        if args.target_return != 0.30:
            config.annual_target_return = args.target_return
        if args.exclude_days != 30:
    ...
    if config.data_source == 'ib':
        print(f"    IB bar size: {config.ib_bar_size}")
    ...
    if args.train:
        print(f"\nTraining enhanced system with {args.days} total days of {data_source.upper()} data")
        print(f"Will exclude recent {args.exclude_days} days for out-of-sample testing")
        
        results = await trading_system.train_system(args.days)
        ...
        print(f"    Data source: {split_info['data_source'].upper()}")
        if split_info.get('ib_bar_size'):
            print(f"    IB bar size: {split_info['ib_bar_size']}")
        ...
        if 'backtest_results' in results:
            ...
            if total_ret > 0.10:
                print(f"\nOUTSTANDING: {total_ret:.1%} return with enhanced {data_source.upper()} system!")
                print("    This system shows potential for strong annual returns")
            elif total_ret > 0.05:
                print(f"\nEXCELLENT: {total_ret:.1%} return achieved with {data_source.upper()} data!")
                print("    Enhanced system is performing well")
            elif total_ret > 0.02:
                print(f"\nGOOD: {total_ret:.1%} return - solid improvement with {data_source.upper()} data")
            elif total_ret > 0:
                print(f"\nPOSITIVE: {total_ret:.1%} return - on the right track")
            else:
                print(f"\nNEEDS WORK: {total_ret:.1%} return - consider parameter tuning")
        ...
        print("Use --train to train the enhanced system")
        print(f"Run parameter optimization first: python parameter_optimizer.py --symbol {symbol} --ib")
