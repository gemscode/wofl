...
class WolfXEnhancedTradingSystem:
    ...
    def load_or_train_all_agents(self, data):
        ...
        print("Training candlestick self-attention agent...")
    ...
    def apply_enhanced_constraints(self, decision, current_price, current_date, force_trade=False):
        """FIXED: Apply enhanced trading constraints - respects forced trades"""
        
        # CRITICAL FIX: If it's a forced trade, bypass most constraints
        if force_trade:
            print(f"[FORCE TRADE] Bypassing constraints for forced {decision}")
            
            # Only check absolute minimum constraints for forced trades
            if decision == 'BUY':
                if self.portfolio['cash'] < 100:  # Minimum $100 for any trade
                    return 'HOLD'
                if self.portfolio['shares'] > 0:  # Already have position
                    return 'SELL'  # Force sell instead
            
            return decision  # Execute the forced trade
        
        # Regular constraint logic for non-forced trades
